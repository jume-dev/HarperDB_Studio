...
describe('Test initLogger', function() {
	...
	it('Test nominal path, create default directory, no errors', async function() {
		...
		assert.ok(winston.default.transports.file !== undefined, "File transport was not initialized.");
		let log_path = `${winston.default.transports.file.dirname}/${winston.default.transports.file.filename}`;
		assert.equal(log_path, default_config.log_location, "log path does not equal the path in config");
		assert.equal(winston.default.transports.console, undefined, "Console logger should have been removed");
	});
	...
	it('Test valid custom log path', async function() {
		...
		assert.ok(winston.default.transports.file !== undefined, "File transport was not initialized.");
		let log_path = `${winston.default.transports.file.dirname}/${winston.default.transports.file.filename}`;
		assert.equal(log_path, custom_log_path, "log path does not equal the path in config");
		assert.equal(winston.default.transports.console, undefined, "Console logger should have been removed");
