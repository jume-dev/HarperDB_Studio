...
//Promisified functions.  We may want to consider bringing in fs-extra or another module that has promisified fs funcs.
...
async function verifyLogLocation(log_path) {
	...
	try {
		...
		console.error(`error getting stats for directory path ${dir_path} ${e}.  Did you create this directory?`);
		return false;
	}
...
async function initLogger() {
	...
	try {
		...
		if(e.code === EXIST_ERR_CODE) {
			...
			console.error(`Got an error trying to create the default log directory at path ${DEFAULT_LOG_LOCATION}.  ${e}`);
			throw new Error(e);
		}
	}
